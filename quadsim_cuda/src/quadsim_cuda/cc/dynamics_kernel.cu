...
__global__ void update_state_vec_cuda_kernel(
    torch::PackedTensorAccessor64<scalar_t,3,torch::RestrictPtrTraits> R_new,
    torch::PackedTensorAccessor64<scalar_t,3,torch::RestrictPtrTraits> R,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> a_thr,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> v_pred,
...
__global__ void run_forward_cuda_kernel(
    torch::PackedTensorAccessor64<scalar_t,3,torch::RestrictPtrTraits> R,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> dg,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> z_drag_coef,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> drag_2,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> pitch_ctl_delay,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> act_pred,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> act,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> p,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> v,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> v_wind,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> a,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> act_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> p_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> v_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> a_next,
...
__global__ void run_backward_cuda_kernel(
    ...
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> v_wind,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> act_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_act_pred,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_act,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_p,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_v,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_a,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> _d_act_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_p_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> d_v_next,
    torch::PackedTensorAccessor64<scalar_t,2,torch::RestrictPtrTraits> _d_a_next,
    float grad_decay,
    float ctl_dt) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    const int B = R.size(0);
...
std::vector<torch::Tensor> run_forward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(R.scalar_type(), "run_forward_cuda", ([&] {
        run_forward_cuda_kernel<scalar_t><<<blocks, threads>>>(
            R.packed_accessor64<scalar_t,3,torch::RestrictPtrTraits>(),
            dg.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            z_drag_coef.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            drag_2.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            pitch_ctl_delay.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            act_pred.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            act.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            p.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            v.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            v_wind.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            a.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            act_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            p_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            v_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            a_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            ctl_dt, airmode_av2a);
    }));
...
std::vector<torch::Tensor> run_backward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(R.scalar_type(), "run_backward_cuda", ([&] {
        run_backward_cuda_kernel<scalar_t><<<blocks, threads>>>(
            ...
            v_wind.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            act_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_act_pred.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_act.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_p.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_v.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_a.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            _d_act_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_p_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            d_v_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            _d_a_next.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            grad_decay, ctl_dt);
    }));
...
torch::Tensor update_state_vec_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(a_thr.scalar_type(), "update_state_vec", ([&] {
        update_state_vec_cuda_kernel<scalar_t><<<blocks, threads>>>(
            R_new.packed_accessor64<scalar_t,3,torch::RestrictPtrTraits>(),
            R.packed_accessor64<scalar_t,3,torch::RestrictPtrTraits>(),
            a_thr.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
            v_pred.packed_accessor64<scalar_t,2,torch::RestrictPtrTraits>(),
